...
void GY_80::update_GY_data(){
  double gy_buffer_pointer_aux[3];
  
  // Eixo X
  gy_buffer_pointer_aux[0] = UpdateAxes(Register_X0, Register_X1);
  // Eixo Y
  gy_buffer_pointer_aux[1] = UpdateAxes(Register_Y0, Register_Y1);
  // Eixo Z
  gy_buffer_pointer_aux[2] = UpdateAxes(Register_Z0, Register_Z1);

  if(gy_buffer_pointer == gy_buffer1){
        memcpy(gy_buffer2, gy_buffer_pointer_aux, sizeof(double)*3);
        gy_buffer_pointer = gy_buffer2;
    }
    else{
        memcpy(gy_buffer1, gy_buffer_pointer_aux, sizeof(double)*3);
        gy_buffer_pointer = gy_buffer1;
    }
}
